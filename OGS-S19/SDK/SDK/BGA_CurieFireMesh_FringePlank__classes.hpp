#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BGA_CurieFireMesh_FringePlank_

#include "Basic.hpp"

#include "BGA_GenericCurieFuel_Parent_classes.hpp"
#include "GameplayAbilities_structs.hpp"


namespace SDK
{

// BlueprintGeneratedClass BGA_CurieFireMesh_FringePlank_.BGA_CurieFireMesh_FringePlank__C
// 0x0028 (0x0978 - 0x0950)
class ABGA_CurieFireMesh_FringePlank__C final : public ABGA_GenericCurieFuel_Parent_C
{
public:
	struct FScalableFloat                         SelfPropagationFuelRowValue;                       // 0x0950(0x0028)(Edit, BlueprintVisible, DisableEditOnInstance)

public:
	void UserConstructionScript();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"BGA_CurieFireMesh_FringePlank__C">();
	}
	static class ABGA_CurieFireMesh_FringePlank__C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABGA_CurieFireMesh_FringePlank__C>();
	}
};
static_assert(alignof(ABGA_CurieFireMesh_FringePlank__C) == 0x000008, "Wrong alignment on ABGA_CurieFireMesh_FringePlank__C");
static_assert(sizeof(ABGA_CurieFireMesh_FringePlank__C) == 0x000978, "Wrong size on ABGA_CurieFireMesh_FringePlank__C");
static_assert(offsetof(ABGA_CurieFireMesh_FringePlank__C, SelfPropagationFuelRowValue) == 0x000950, "Member 'ABGA_CurieFireMesh_FringePlank__C::SelfPropagationFuelRowValue' has a wrong offset!");

}

