#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: QuartzMusic_IOandSeven

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "QuartzMusicTrackComponent_DataTracker_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass QuartzMusic_IOandSeven.QuartzMusic_IOandSeven_C
// 0x0010 (0x01F8 - 0x01E8)
class UQuartzMusic_IOandSeven_C final : public UQuartzMusicTrackComponent_DataTracker_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_QuartzMusic_IOandSeven_C;           // 0x01E8(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	float                                         UnawareValue;                                      // 0x01F0(0x0004)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

public:
	void ExecuteUbergraph_QuartzMusic_IOandSeven(int32 EntryPoint);
	float GetBaseValue();
	void GetDataTrackerParameter(int32 DataTrackerIndex, float* ReleaseSpeed);
	float GetIOValue();
	float GetSevenValue();
	float GetUnawareValue();
	void MIx_State();
	void OnDataTrackerUpdate();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"QuartzMusic_IOandSeven_C">();
	}
	static class UQuartzMusic_IOandSeven_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UQuartzMusic_IOandSeven_C>();
	}
};
static_assert(alignof(UQuartzMusic_IOandSeven_C) == 0x000008, "Wrong alignment on UQuartzMusic_IOandSeven_C");
static_assert(sizeof(UQuartzMusic_IOandSeven_C) == 0x0001F8, "Wrong size on UQuartzMusic_IOandSeven_C");
static_assert(offsetof(UQuartzMusic_IOandSeven_C, UberGraphFrame_QuartzMusic_IOandSeven_C) == 0x0001E8, "Member 'UQuartzMusic_IOandSeven_C::UberGraphFrame_QuartzMusic_IOandSeven_C' has a wrong offset!");
static_assert(offsetof(UQuartzMusic_IOandSeven_C, UnawareValue) == 0x0001F0, "Member 'UQuartzMusic_IOandSeven_C::UnawareValue' has a wrong offset!");

}

